			...
			<td>
				...
				<span class="subforums">Subforums:
					<?php foreach($forum['ChildForum'] as $key => $ChildForum) : ?>
						<?php echo $html->link($ChildForum['ForumForum']['title'],array('controller' => 'forums', 'plugin' => 'forums', 'action' => 'forum', $ChildForum['ForumForum']['slug'])); if($key < (count($forum['ChildForum'])-1)) echo ", ";?>
					<?php endforeach; ?>
				</span>
				<?php endif;?>
			</td>
